...
import { generateMirrorBracket } from "@/lib/tournament/generate-bracket"
...
export async function POST(
  ...
  try {
    ...
    const result = await generateMirrorBracket(id, modId, session!.user.id)
    return NextResponse.json({ success: true, data: result })
  } catch (err) {
    console.error("Error generating mirror bracket:", err)
    const message = err instanceof Error ? err.message : "Error al generar eliminatorias"
