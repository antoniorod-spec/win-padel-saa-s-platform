...
function ClubesPageContent() {
  ...
  const cities = useMemo(() => {
    ...
    return Array.from(new Set(all)).sort((a, b) => (cityLabels[a] ?? a).localeCompare(cityLabels[b] ?? b))
  ...
  const clubsWithCoordinates = clubs.filter(
    (club) => typeof club.latitude === "number" && typeof club.longitude === "number"
  )
  const mapCenter =
    clubsWithCoordinates[0] && typeof clubsWithCoordinates[0].latitude === "number" && typeof clubsWithCoordinates[0].longitude === "number"
      ...
      <main>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 text-muted-foreground">
                  Encuentra clubes por ciudad, superficies y servicios disponibles.
                </p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <div className="flex rounded-lg border border-border/70 bg-muted/40 p-1">
                  ...
                  <Button
                    ...
                    title={!clubsDirectoryMapEnabled ? "Mapa deshabilitado por admin" : "Ver mapa"}
                  >
          ...
          <div className="grid gap-6 lg:grid-cols-12">
                ...
                <CardContent className="space-y-5 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setSearchInput("")
                        router.replace("/clubes", { scroll: false })
                      }}
                    >
                      Limpiar
                    </button>
                  ...
                  <div className="space-y-2">
                    ...
                    {seoCitySlug && cityFilter ? (
                      ...
                      >
                        Ver torneos en {selectedCityLabel}
                      </Link>
                    ) : null}
                  </div>

                  <div>
                    <p className="mb-2 text-sm font-medium">Tipo de cancha</p>
                    <div className="space-y-2">
                      {surfaceOptions.map((surface) => (
                        <label key={surface} className="flex items-center gap-2 text-sm">
                          <input
                            ...
                            onChange={() => toggleCsvParam("surfaces", surface)}
                          />
                          {surface}
                        </label>
                      ))}
            ...
            <div className="space-y-4 lg:col-span-9">
              <div className="flex items-center justify-between text-sm text-muted-foreground">
                <p>{isLoading ? "Cargando clubes..." : `${total} clubes encontrados`}</p>
                <p>
                  ...
                  <CardContent className="p-4">
                    {!clubsDirectoryMapEnabled ? (
                      <p className="text-sm text-muted-foreground">
                        El mapa esta deshabilitado temporalmente por el administrador. El listado filtrado sigue disponible.
                      </p>
                    ) : !hasMapsApiKey ? (
                      <p className="text-sm text-muted-foreground">
                        Para visualizar el mapa agrega `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`. El listado filtrado sigue disponible.
                      </p>
                    ) : mapLoadError ? (
                      <p className="text-sm text-muted-foreground">No se pudo cargar el mapa en este momento.</p>
                    ) : !isMapLoaded ? (
                      <p className="text-sm text-muted-foreground">Cargando mapa...</p>
                    ) : clubsWithCoordinates.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No hay clubes con coordenadas para este filtro.</p>
                    ) : (
                      <GoogleMap
        ...
        <section className="mt-10 bg-primary py-12">
          <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-4 text-center md:flex-row md:text-left lg:px-8">
            <div>
              <h2 className="font-display text-3xl font-black uppercase text-primary-foreground md:text-4xl">
                ¿Eres dueño de un club?
              </h2>
              <p className="mt-2 text-primary-foreground/90">
                Únete a WhinPadel y aumenta la visibilidad de tus canchas y torneos.
              </p>
            </div>
            <Link href="/registro">
